...
#Imortando o gerenciador de conexões
...
#rota para criação de invites de participantes linkando com o bd de emails
@trips_routes_bp.route("/trips/<tripId>/invites", methods=["POST"])
def invite_to_trip(tripId):
    conn = db_connection_handler.get_connection()
    participants_repository = ParticipantsRepository(conn)
    emails_repository = EmailsToInviteRepository(conn)
    controller = ParticipantCreator(participants_repository, emails_repository) #injeção de dependência
    
    response = controller.create(request.json, tripId) #request.json => devolve informações de body
    
    return jsonify(response["body"]), response["status_code"]

#rota para criação de activities
